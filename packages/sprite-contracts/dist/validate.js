...
function validateAnimationEvent(value, frameCount) {
    if (!isObject(value))
        return false;
    return (isNonNegativeInteger(value.frame) &&
        Number(value.frame) < frameCount &&
        typeof value.kind === 'string' &&
        (value.name === undefined || typeof value.name === 'string') &&
        (value.payload === undefined || isObject(value.payload)));
}
function validateAnimation(value, totalFrames) {
    if (!isObject(value))
        return false;
    if (!isPositiveInteger(value.frames))
        return false;
    if (!isPositiveInteger(value.fps))
        return false;
    const frameCount = Number(value.frames);
    if (value.order !== undefined) {
        if (!Array.isArray(value.order) || value.order.length !== frameCount)
            return false;
        for (const frame of value.order) {
            if (!isNonNegativeInteger(frame) || Number(frame) >= totalFrames)
                return false;
        }
    }
    ...
    for (const key of ['hitboxes', 'hurtboxes']) {
        if (value[key] !== undefined) {
            if (!Array.isArray(value[key]) || !value[key].every(validateBox))
                return false;
        }
    }
...
function getSourceSheets(source) {
...
export function validateSpriteManifest(manifest) {
    ...
    for (const sheet of sheets) {
        ...
        if (typeof sheet.file !== 'string' || sheet.file.length === 0)
            return false;
        if (!Array.isArray(sheet.frameSize) || sheet.frameSize.length !== 2)
            return false;
        if (!isPositiveInteger(sheet.frameSize[0]) || !isPositiveInteger(sheet.frameSize[1]))
            return false;
        let totalGridFrames;
        if (sheet.grid !== undefined) {
            if (!isObject(sheet.grid))
                return false;
            if (!isPositiveInteger(sheet.grid.columns) || !isPositiveInteger(sheet.grid.rows))
                return false;
            totalGridFrames = Number(sheet.grid.columns) * Number(sheet.grid.rows);
        }
        if (!isObject(sheet.animations))
            return false;
        ...
        for (const anim of Object.values(animations)) {
            ...
            if (!validateAnimation(anim, totalFrames))
                return false;
        }
    }
    return true;
