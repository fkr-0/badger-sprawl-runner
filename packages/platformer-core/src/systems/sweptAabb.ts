...
function sweptTime(body: Rect, obstacle: SweptObstacle, vx: number, vy: number, dt: number): SweepHit | null {
	...
	if (obstacle.oneWay && body.y + body.h > obstacle.y + 0.001) return null;
...
export function sweepAabb(input: SweepInput): SweepResult {
	if (!Number.isFinite(input.dt) || input.dt < 0) throw new Error(`Invalid sweep dt: ${input.dt}`);
