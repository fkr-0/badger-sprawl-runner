...
function inverseMass(body: ImpulseBody): number {
	if (body.invMass !== undefined) return Math.max(0, body.invMass);
...
function contactKey(contact: CollisionContact): string {
	return contact.a < contact.b ? `${contact.a}\u0000${contact.b}` : `${contact.b}\u0000${contact.a}`;
}

export function resolveImpulseCollisions<T extends ImpulseBody>(
	bodies: readonly T[],
