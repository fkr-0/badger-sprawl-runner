...
function normalize(dx: number, dy: number): { x: number; y: number } {
	...
	if (length === 0) throw new Error('Cannot raycast with a zero direction');
	return { x: dx / length, y: dy / length };
}

function rayAabb(input: RaycastInput, obstacle: RaycastObstacle, nx: number, ny: number): RaycastHit | null {
	...
	if (nx === 0) {
		if (input.x < obstacle.x || input.x > obstacle.x + obstacle.w) return null;
		t1 = Number.NEGATIVE_INFINITY;
		t2 = Number.POSITIVE_INFINITY;
	}
	if (ny === 0) {
		if (input.y < obstacle.y || input.y > obstacle.y + obstacle.h) return null;
	...
	if (tEnter > tExit || tExit < 0 || tEnter > input.maxDistance) return null;

	const distance = Math.max(0, tEnter);
	let normalX = 0;
	let normalY = 0;
	if (tMinX > tMinY) normalX = nx > 0 ? -1 : 1;
	else normalY = ny > 0 ? -1 : 1;
...
export function raycast(input: RaycastInput): RaycastHit | null {
	if (!Number.isFinite(input.maxDistance) || input.maxDistance < 0) throw new Error(`Invalid raycast maxDistance: ${input.maxDistance}`);
	...
	return input.obstacles
		...
		.filter((hit): hit is RaycastHit => hit !== null)
		.sort((a, b) => a.distance - b.distance || a.obstacle.id.localeCompare(b.obstacle.id))[0] ?? null;
}

export function hasLineOfSight(input: RaycastInput): boolean {
