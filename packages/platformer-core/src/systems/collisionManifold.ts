...
function ordered(a: ManifoldBody, b: ManifoldBody): [ManifoldBody, ManifoldBody] {
	return a.id <= b.id ? [a, b] : [b, a];
}

export function computeCollisionManifold(aBody: ManifoldBody, bBody: ManifoldBody): CollisionManifold | null {
...
export function manifoldsFromSpatialPairs(pairs: readonly SpatialPair[]): CollisionManifold[] {
	return pairs
		.map((pair) => computeCollisionManifold(pair.a, pair.b))
		.filter((manifold): manifold is CollisionManifold => manifold !== null)
