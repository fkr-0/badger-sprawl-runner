...
export class Renderer {
  ...
  renderPlatforms(platforms: Array<{ x: number; y: number; w: number; h: number }>, cameraX: number): void {
  ...
  renderPickups(pickups: Array<{ x: number; y: number; kind: string; taken: boolean }>, cameraX: number): void {
  ...
  private getPickupColor(kind: string): string {
    switch (kind) {
      case 'rocket': return '#ffb35e';
      case 'railgun': return '#67f3c4';
      case 'stim': return '#ff5e7a';
      case 'katana': return '#eaf2ff';
      default: return '#fff';
    }
  }
}
