...
import { stepDeterministicRun, type DeterministicRunState } from './DeterministicRunStep';
...
function world(): PhysicsWorldState {
	return {
		...
		projectiles: [{
			id: 'shot', kind: 'rail', ownerId: 'player', x: 40, y: 25, vx: 200, vy: 0, angle: 0, angularVelocity: 0,
			mass: 1, radius: 4, life: 1, damage: 2, pierce: 0, active: true, bounces: 0, maxBounces: 0, tags: [],
		}],
		platforms: [],
		materialZones: [{
			x: -10, y: 0, w: 50, h: 80,
			material: { id: 'acid', friction: 1, traction: 1, restitution: 0, damagePerSecond: 4, tags: ['hazard', 'acid'] },
		}],
...
function state(): DeterministicRunState {
	return {
		physics: world(),
		combatants: [combatant({ id: 'player', hp: 3 }), combatant({ id: 'drone', faction: 'enemy', x: 45, y: 10, hp: 5 })],
		items: [{ actorId: 'player', definition: heal, state: createItemUseState(heal), requestedUse: true }],
		tick: 0,
		time: 0,
	};
}
...
describe('DeterministicRunStep', () => {
	it('replays the same integrated frame into the same state hash', () => {
		const first = stepDeterministicRun({ state: state(), params: defaultParams, dt: 0.1, projectileAttackerId: 'player' });
		const second = stepDeterministicRun({ state: state(), params: defaultParams, dt: 0.1, projectileAttackerId: 'player' });
	...
	it('applies active item effects, material hazard damage, and projectile combat damage in one deterministic frame', () => {
		const result = stepDeterministicRun({
			...
			statusByMaterialTag: {
				acid: [{ id: 'acid-burn', kind: 'burn', sourceId: 'acid', duration: 1, remaining: 1, stacks: 1, maxStacks: 1, tickInterval: 0.5, tickTimer: 0.5, magnitude: 1 }],
			},
		});
