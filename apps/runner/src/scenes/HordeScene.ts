...
export class HordeScene implements Scene {
  ...
  update(dt: number): void {
    ...
    this.combat.step(this.player, this.enemySystem.getEnemies(), action, dt, {
      ...
      requestHitstop: (duration) => { /* hitstop */ },
      requestScreenShake: (intensity) => { /* screen shake */ },
    });
    ...
    if (!this.waveDirector.isWaveActive() && this.waveDirector.getCurrentWave() < this.waveDirector.getTotalWaves()) {
    ...
    // Check victory
    if (this.waveDirector.getCurrentWave() >= this.waveDirector.getTotalWaves() &&
  ...
  private renderHordeUI(ctx: CanvasRenderingContext2D): void {
    ...
    if (this.victory) {
      ...
      ctx.fillText('Press ENTER to return to menu', ctx.canvas.width / 2, ctx.canvas.height / 2 + 40);
    }
    ...
    if (this.gameOver) {
      ...
      ctx.font = '18px ui-monospace, monospace';
      ctx.fillText(`Reached Wave ${this.waveDirector.getCurrentWave()}`, ctx.canvas.width / 2, ctx.canvas.height / 2 + 40);
