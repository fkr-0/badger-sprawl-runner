...
import { playAnimation, advanceAnimation, createAnimationState, type AnimationState } from '../renderer/AnimationState';
...
export class StageRunScene implements Scene {
  ...
  update(dt: number): void {
    ...
    this.combat.step(this.player, this.enemies, action as Record<string, boolean>, simDt, {
      ...
      requestHitstop: (duration) => { this.hitstopRemaining = duration; },
      requestScreenShake: (intensity) => { this.screenShakeIntensity = intensity; },
    });
    // 5. Items
  ...
  render(renderer: unknown, alpha: number): void {
    ...
    const shakeX = this.screenShakeIntensity > 0
      ? (Math.random() - 0.5) * this.screenShakeIntensity * 2
      : 0;
    const shakeY = this.screenShakeIntensity > 0
      ? (Math.random() - 0.5) * this.screenShakeIntensity * 2
      : 0;
