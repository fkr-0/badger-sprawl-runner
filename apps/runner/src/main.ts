...
function drawStage(): void {
  ...
  title(stage?.name ?? 'STAGE', `stage ${(state.mode === 'stage' ? state.stageIndex : 0) + 1} of 2`);
...
function drawSkills(): void {
  ...
  skillIds.forEach((skillId, index) => {
    ...
    ctx.fillStyle = '#92a4be';
    ctx.font = '13px ui-monospace, monospace';
    ctx.fillText(unlocked ? 'learned' : 'press Enter to unlock if prerequisites and shards allow', 240, y + 22);
  });
...
function updatePanels(): void {
  ...
  miniPanel.innerHTML = '<strong>Controls:</strong> Arrow keys navigate. Enter/Space confirms. Escape returns to menu.<br/><strong>Implemented slice:</strong> two stages, dialogue, VS shell, dummy training, skill tree.';
}
