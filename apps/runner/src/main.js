...
function control(dt) {
  ...
  if (!down('Space', 'KeyW', 'ArrowUp') && player.vy < -120)
    player.vy *= P.variableJumpCut;
...
function melee() {
  ...
  const box = {
    x: player.x + (player.dir > 0 ? player.w : -42),
    y: player.y + 8,
    w: 42,
    h: 28,
  };
...
function useItem() {
  if (player.hasRocket && player.fuel > 0 && player.boostCd <= 0) {
    ...
    const side =
      ((down('KeyD', 'ArrowRight') ? 1 : 0) - (down('KeyA', 'ArrowLeft') ? 1 : 0)) ||
      player.dir;
    player.vx += side * 230;
...
function collect(p) {
  ...
  if (p.kind === 'stim') {
    player.stims += 2;
    world.message =
      'Stim packs +2. E uses one when hurt if no rocket boost is ready.';
  }
  if (p.kind === 'katana') {
