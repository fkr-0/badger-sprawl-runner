...
assert(Array.isArray(progression.currencies) && progression.currencies.length >= 4, 'expected progression currencies');
assert(Array.isArray(sprites.spriteSheets) && sprites.spriteSheets.length >= 4, 'expected sprite sheets');
...
for (const sheet of sprites.spriteSheets) {
  ...
  assert(Array.isArray(sheet.frameSize) && sheet.frameSize.length === 2, `bad frameSize for ${sheet.id}`);
  assert(Object.keys(sheet.animations || {}).length > 0, `sprite sheet has no animations: ${sheet.id}`);
