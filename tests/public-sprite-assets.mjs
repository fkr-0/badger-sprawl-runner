...
function pngDimensions(buffer) {
	...
	assert(buffer.subarray(0, 8).equals(Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a])), 'invalid PNG signature');
...
assert(
	(await fileHash('data/sprites.json')) === (await fileHash('apps/runner/public/data/sprites.json')),
...
for (const sheet of sourceManifest.spriteSheets) {
	...
	assert(actualWidth === expectedWidth, `sprite asset width mismatch for ${sheet.id}: ${actualWidth} !== ${expectedWidth}`);
	assert(actualHeight === expectedHeight, `sprite asset height mismatch for ${sheet.id}: ${actualHeight} !== ${expectedHeight}`);
	checked++;
}
