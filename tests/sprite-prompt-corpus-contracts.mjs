...
assert.match(promptIndex, /remaining_gap_catalog: llm-sprite-generation\/remaining-gaps\.yml/);
assert.match(
	promptIndex,
	/render_job_manifest: docs\/sprite-production\/render-jobs\/manifest\.json/
);

const gapCatalog = readFileSync(join(root, 'llm-sprite-generation', 'remaining-gaps.yml'), 'utf8');
