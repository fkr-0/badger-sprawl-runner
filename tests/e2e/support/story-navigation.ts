...
export async function enterStoryFlow(page: Page): Promise<void> {
	...
	await page.evaluate(() => {
		const stageId = window.__badger?.getStoryProgress()?.currentStageId ?? 'lower-sprawl';
		...
		if (!scene?.selectLocation?.(routeId)) {
			throw new Error(`SubwayMapScene could not select the active story route ${routeId}`);
		}
